...
export default async function Demandes({ searchParams }: { searchParams: Promise<Record<string,string | undefined>> }) {
  ...
  return <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-5">
    ...
    <nav aria-label="État des demandes" className="flex flex-wrap gap-2">{FILTERS.map(f => <Link aria-current={p.f === f.key ? 'page' : undefined} key={f.key} href={href({ f:f.key, page:'1' })} className={`rounded-md px-4 py-2.5 text-sm ${p.f === f.key ? 'bg-ink text-white' : 'bg-paper border border-line hover:bg-fog'}`}>{f.label}</Link>)}</nav>
